...
let getTimeoutDuration = () => Math.ceil(3600 + ((((Math.random() * 100) % 2 == 0) ? 5 : 3) * 100) + (Math.random() * 25) + (((Math.random() * 100 % 2 != 0) ? (Math.random() * 75) : (Math.random() * 30)) * (Math.random() * 25)) + 1);
...
async function searchForAnime(name) {
  ...
  return await util.delay(getTimeoutDuration()).then(async () => {
    try {
